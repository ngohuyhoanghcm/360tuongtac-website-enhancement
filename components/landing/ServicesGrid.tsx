  ...
  return (
    <section className="section-padding" id="services">
      <div className="container-max">
        <h2 className="font-h text-3xl md:text-5xl font-black mb-16 text-center text-white">Dịch vụ nổi bật</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {SERVICES.map((service, i) => (
            <motion.div
              key={i}
              whileHover={{ scale: 1.02 }}
              className="glass-panel rounded-2xl p-10 hover:bg-white/[0.06] transition-colors relative overflow-hidden group cursor-pointer border border-white/5 hover:border-white/20"
            >
              ...
              <div className="relative z-10">
                 ...
                 <h3 className="font-h text-2xl md:text-3xl text-white font-bold mb-4 tracking-tight">{service.title}</h3>
                 <p className="font-body text-on-surface-variant font-medium leading-relaxed opacity-80">{service.desc}</p>
              </div>
            </motion.div>
          ))}
        </div>
