...
export default function CaseStudyGrid() {
  return (
    <section className="py-16 md:py-24 px-6 border-t border-white/5 bg-background-dark">
      <div className="container-max">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {caseStudies.map((study, index) => (
            <div key={index} className="glass-panel p-8 rounded-xl border border-white/10 hover:border-white/20 transition-all duration-300 group">
              <div className="flex justify-between items-start mb-6">
                <div>
                  <h3 className="font-h text-xl font-bold text-white mb-1">{study.name}</h3>
                ...
                <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center">
                  <study.icon className="w-5 h-5 text-text-secondary group-hover:text-primary transition-colors" />
                </div>
              </div>

              <div className="flex flex-col gap-3 py-6 border-y border-white/10 mb-6">
                <div className="flex justify-between items-center">
                  <span className="text-sm text-text-secondary">Trước khi áp dụng:</span>
                  <span className="text-sm font-medium text-white">{study.initialState}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-text-secondary">Kết quả hiện tại:</span>
                  <span className="text-sm font-bold text-accent">{study.finalState}</span>
                </div>
              </div>

              <p className="font-body text-text-secondary leading-relaxed text-sm mb-6 min-h-[80px]">
                {study.description}
              </p>

              <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-3 text-center">
                <span className="text-green-400 font-bold font-h">{study.result}</span>
              </div>
            </div>
          ))}
        </div>
