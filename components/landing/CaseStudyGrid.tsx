      ...
      <div className="container-max">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {caseStudies.map((study, index) => (
            <div key={index} className="bg-white p-8 rounded-xl border border-gray-200 hover:border-gray-300 transition-all duration-300 group shadow-md hover:shadow-lg">
              <div className="flex justify-between items-start mb-6">
                <div>
                  <h3 className="font-h text-xl font-bold text-gray-900 mb-1">{study.name}</h3>
                ...
                <div className="w-10 h-10 rounded-full bg-gray-50 flex items-center justify-center">
                  <study.icon className="w-5 h-5 text-gray-600 group-hover:text-[#FF8C00] transition-colors" />
                </div>
              </div>

              <div className="flex flex-col gap-3 py-6 border-y border-gray-200 mb-6">
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-600">Trước khi áp dụng:</span>
                  <span className="text-sm font-medium text-gray-900">{study.initialState}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-600">Kết quả hiện tại:</span>
                  <span className="text-sm font-bold text-[#FF2E63]">{study.finalState}</span>
                </div>
              </div>

              <p className="font-body text-gray-600 leading-relaxed text-sm mb-6 min-h-[80px]">
                {study.description}
              </p>

              <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-3 text-center">
                <span className="text-green-700 font-bold font-h">{study.result}</span>
              </div>
            </div>
          ))}
        </div>
