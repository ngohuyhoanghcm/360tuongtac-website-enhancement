        ...
        <div className="max-w-3xl mx-auto space-y-4">
          {displayFaqs.map((faq, i) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="glass-panel border border-white/5 rounded-2xl overflow-hidden hover:border-white/10 transition-colors"
            >
              <button
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <span className="font-h1 text-lg font-bold text-white antialiased">{faq.q}</span>
                </div>
                <ChevronDown 
                   size={20} 
                   className={`text-slate-500 transition-transform duration-300 ${openIndex === i ? 'rotate-180' : ''}`} 
                />
              </button>
              ...
              {openIndex === i && (
                <motion.div
                  ...
                  className="px-6 pb-6"
                >
                  <p className="font-body text-slate-400 leading-relaxed font-medium pt-2 border-t border-white/5">
                    {faq.a}
                  </p>
                </motion.div>
              )}
