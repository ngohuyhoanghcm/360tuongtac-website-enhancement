...
// Map slugs to their specific prompt ideas given by the user
const tasks = [
...
async function run() {
  for (const task of tasks) {
    ...
    try {
      ...
      // Ensure binary saving, no utf-8 mishmash
  ...
  for (const filename of files) {
    ...
    // Find the current featuredImage and replace it
    ...
    // Replace alt tag to be Vietnamese AEO-standard alt tags
